...
// Same-origin base URLs (preview compatible)
var API_BASE = "/api/";
...
function getData(data, l1, l2) {
  if (!isSort) {
    ...
    $.getJSON(API_BASE + data, function (result) {
      ...
      $.getJSON(API_BASE + data + "?limit1=" + l1 + "&limit2=" + l2, function (result2) {
        makeTable(result2, l1, l2);
      });
    });
  } else {
    buildLink(l1, l2);
...
function makeTable(result, l1, l2) {
  ...
  table +=
    '<div class="col-xl-3 col-md-6 mb-4"><div class="card border-left-success shadow h-10 py-2"><div class="card-body"><div class="row no-gutters align-items-center"><div class="col mr-2"><div class="text-xs font-weight-bold text-success text-uppercase mb-1">Number of ' +
    tablename +
  ...
  if (tablename == "jobs") {
    table +=
      ...
      sortSelected[0] +
      'value="pending">Pending</option><option ' +
      sortSelected[1] +
      ' value="delivered">Delivered</option><option ' +
      sortSelected[2] +
  ...
  if (l2 == 0) {
    if (result.length < 10) {
      ...
      table +=
        ...
        "," +
        l2 +
        ');changePageNum(true);"><span class="badge badge-primary"><i class="fas fa-angle-right"></i></span></h3><div style="width:150px;">Page ' +
        currentPage +
        " of " +
        totalPages +
        "</div></div></div></footer>";
...
function makeModals(result) {
  ...
  modal +=
    ...
    tablename +
    '\');">Create Record</button></div></div></div></div>';
...
function changePassword() {
  ...
  if (newpas1 == newpas2) {
    $.ajax({
      method: "PUT",
      url: ADMIN_BASE + "changepassword?oldPassowrd=" + encodeURIComponent(oldpas) + "&newPassword=" + encodeURIComponent(newpas1),
      success: function (dat) {
        if (dat.Status == "Error") {
...
function buildLink(l1, l2) {
  ...
  if (option == "all") {
    ...
    $.getJSON(API_BASE + link.toLowerCase(), function (result) {
      totalNum = result.length;
      $.getJSON(API_BASE + link.toLowerCase() + "?limit1=" + l1 + "&limit2=" + l2, function (result2) {
        makeTable(result2, l1, l2);
      });
    });
  }
